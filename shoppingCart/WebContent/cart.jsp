		...
		<table class="table table-light">
			...
			<tbody>
				...
				<tr>
					...
					<!-- for loop for each added item in cart  -->

					<td>
						<form action="" method="post" class="form-inline">
							<input type="hidden" name="id" value="<%=c.getItmnbr()%>"
								class="form-input">
							<div class="form-group d-flex justify-content-between">

								<a class="btn btn-small btn-incre"
									href="quantity-inc-dec?action=inc&id=<%=c.getItmnbr()%>"><i
									class="fas fa-plus-square"></i> </a>
								<!-- plus button -->
								<input type="text" name="quantity" class="form-control"
									value="<%=c.getQuantity()%>" readonly>
								<!-- Anzeigefeld Menge -->
								<a class="btn btn-small btn-decre"
									href="quantity-inc-dec?action=dec&id=<%=c.getItmnbr()%>"><i
